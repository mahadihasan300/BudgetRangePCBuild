...
$(function() {
	...
	$('.report-trigger').on(
			...
			function(e) {
				...
				var htmlViewer = document.getElementById("report-viewport");
				...
				if ('pdf' === this_trigger.val()) {
					...
					$.ajax({
						type : 'POST',
						url : reportFr.attr('action') + "/" + mode, // grab URL
																	// from the
																	// form's
																	// action
																	// attribute
						...
						success : function(response) {
							...
							pdfViewer.src = base_url + reportFr.attr('action')
									+ "?" + values;
						},
					...
					// $.post(reportFr.attr('action') + "/" + mode, values,
					// function(res) {
					// pdfViewer.src = base_url + reportFr.attr('action') + "?"
					// + values;
					// // pdfViewer.src =
